...
//Rutina que implementa el algoritmo de Kessels
void proceso_hilo_Kessels()
{
	int id = omp_get_thread_num();
	//Levantamos la bandera de que estamos por entrar a la
	//seccion critica
	b_k[id] = 1;
	local_k[id] = (turno_k[1-id]+id)%2;
	turno_k[id] = local_k[id];
	while(!(b_k[1-id] == 0 || local_k[id] != (turno_k[1 -id] +id)%2)){}
	//Si alguna de las dos condiciones anteriores no se cumple ejecutamos la seccion
	seccion_critica();
	//Acabando bajamos la bandera de que ocupamos la seccion critica
	//Permitiendo al otro hilo usar la seccion
	b_k[id] = 0;
}
...
int main(int argc, char** argv)
{
	...
	omp_set_num_threads(2);
	printf("Proceso con Kessels \n");
	#pragma omp parallel
	{
		int j = 0;
		while(j<20){
			proceso_hilo_Kessels();
			j += 1;
		}
	}
	return 0;
